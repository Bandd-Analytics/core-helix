...
Port of V1/helix/src/alpha/regime/hmm_garch.py with two surgical changes per
CONTEXT.md D-04:

  - predict_viterbi method removed (no Viterbi anywhere in V2).
  - _compute_log_emission_probs helper removed (only consumer was predict_viterbi).
...
Two-stage fit:
  ...
  2) Per-state arch_model GARCH(1,1) -> GARCHParams; states sorted ascending
     by unconditional_variance (REGM-02), transmat_/startprob_ remapped via
     `_remap_matrix`.
"""
...
class HMMGARCHRegimeDetector:
    ...
    def fit(self, returns: np.ndarray) -> bool:
        ...
        # Sort states by ascending unconditional variance (REGM-02)
