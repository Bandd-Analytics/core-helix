...
//|                                                                   |
//|  MQ5 v2.00 port. Session H/L–bounded boxes via iHigh/iLow lookup, |
//|  London/NY changeover gap boxes, optional AR Line, pip-range      |
//|  labels. Reads broker offset via "sm_GMTOffset" GlobalVariable    |
//|  (auto-GMT variant — D-19 distinction).                           |
//|                                                                   |
//|  Note: MQ4 OBJ_RECTANGLE has no native alpha-blend; opacity input |
//|  is exposed for forward compatibility but the rendered fill is    |
//|  solid. Tradeoff vs MQ5 ColorToARGB.                              |
//+------------------------------------------------------------------+
...
input int    InpAsiaOpacity    = 20;     // advisory only on MQ4
...
input bool   InpUseGMTOffset   = false;
...
void ReadOffset()
  {
   if(!InpUseGMTOffset) { g_offsetSec = 0; return; }
...
//+------------------------------------------------------------------+
//  Find session H/L by iterating bars in [t1, t2]
//+------------------------------------------------------------------+
...
int PeriodSeconds()
  {
   //--- MQ4 helper — Period() returns minutes
   return(Period() * 60);
  }
