...
- [ ] **BRDG-01**: Versioned msgpack schema contract file defines Tick, Bar, OrderRequest, Fill, Heartbeat types before any bridge code is written
...
- [ ] **REGM-01**: HMM-GARCH classifier ported from V1 to V2/v3_intelligence/regime.py with offline fit + online update split
- [ ] **REGM-02**: HMM state labels pinned by variance rank at fit time (prevents permutation when re-fitting on new dataset)
